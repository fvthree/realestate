...
export const publishProperty = (id: string) => api.create(`${url.PUBLISH_PROPERTY}/${id}/publish`, {});
...
// Inquiries
export const createInquiry = (propertyId: string, payload: any) =>
  api.create(`${url.CREATE_INQUIRY}/${propertyId}/inquiries`, payload);
export const getBuyerInquiry = (inquiryId: string, token: string) =>
  api.get(`${url.GET_BUYER_INQUIRY}/${inquiryId}`, { token });
export const updateBuyerInquiry = (inquiryId: string, token: string, payload: any) =>
  api.update(`${url.UPDATE_BUYER_INQUIRY}/${inquiryId}?token=${encodeURIComponent(token)}`, payload);
...
export const addAgentInquiryMessage = (inquiryId: string, payload: any) =>
  api.create(`${url.ADD_AGENT_MESSAGE}/${inquiryId}/messages`, payload);
